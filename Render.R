...
rmarkdown::render(input = "Reporting.Rmd",
                  ...
                  output_file = glue("Reports/Adelie-Report.html"))


render_fn<-function(sp){
        rmarkdown::render(input = "Reporting.Rmd",
                          params = list(species=sp),
                          output_file = glue("Reports/report-{sp}.html"
                                             ))
}
render_fn("Chinstrap")
...
paths<-paste0("Reports/",species,".html")
...
paths2<-cross2(species,category) %>% list.stack() %>% unite(sep = "-",col = "path",remove = T) %>% 
        map(~paste0("Reports/",.x,".html")) %>% flatten()
...
category<-data %>% select(where(is.factor)) %>% select(-1) %>% names()
paths2<-list.expand(species,category) %>% 
        list.stack() %>%
        unite(sep = "-",col = "path",remove = T) %>% 
        map(~paste0("Reports/",.x,".pdf")) %>% 
        flatten()
